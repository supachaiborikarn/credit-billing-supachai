...
### GAS Admin Shift Data Entry Without Staff Open (Apr 28, 2026)
- **ปัญหา**: แอดมินมีหน้าใส่ข้อมูลย้อนหลังอยู่แล้ว แต่ยังไม่ชัดว่าใช้สร้างกะแทนพนักงานได้ และยังไม่ครบข้อมูลกะจริง เช่น ราคาก๊าซ, สถานะ OPEN/CLOSED, เกจเปิด/ปิด, ยอดขายอื่น, ค่าใช้จ่าย, reconciliation และ audit
- **แก้ไข**: ยกระดับ `/admin/gas/data-entry` ให้เลือกปั๊ม/วันที่/กะ แล้วสร้างหรือแก้กะจากแอดมินได้โดยตรง; เลือกได้ว่าจะบันทึกเป็นกะ `OPEN` เพื่อให้พนักงานทำต่อ หรือ `CLOSED` เพื่อสร้างมิเตอร์/เกจ/ยอดขาย synthetic/admin-data-entry transactions และ `ShiftReconciliation` ครบในครั้งเดียว
- **Safety**: ถ้าจะบันทึกเป็น `OPEN` ระบบบล็อกกรณีมีกะอื่นเปิดอยู่ในวันเดียวกัน หรือกะนั้นเคยกระทบยอดแล้ว; การแก้ยอดขายจะ replace เฉพาะ transaction ที่ notes ขึ้นต้น `admin-data-entry:` ไม่แตะรายการขายจริงของพนักงาน
- **สถานะ**: ✅ patch route/page/tests แล้ว; ปุ่มจาก `/admin/gas/operations` พาเข้า data-entry แทนต้องไปเปิดกะที่หน้าพนักงาน

### GAS Staff Open Shift Silent Submit Guard (Apr 28, 2026)
- **ปัญหา**: production logs เห็นพนักงานเข้า `/gas/6/shift/open` และ `/api/v2/gas/6/shift/current` แต่ไม่เห็น `POST /api/v2/gas/6/shift/open`; เมื่อกดเปิดกะแล้วติด validation/client-side issue จึงดูเหมือนกดแล้วไม่ไปต่อ โดยเฉพาะ mobile ที่ error อยู่เหนือปุ่ม
- **แก้ไข**: ปรับหน้า `/gas/[stationId]/shift/open` ให้ใช้ form submit ที่ชัดเจน, validate มิเตอร์/เกจ/ราคาอีกครั้งก่อนส่ง, เลื่อนจอไปหา error อัตโนมัติ, อ่าน error body ที่ไม่ใช่ JSON ได้, ใส่ timeout กัน pending เงียบ, และรับเลขมี comma/เลขไทยผ่าน `inputMode=decimal`
...
23. **GAS Admin Analytics Source of Truth**: daily/shift/reconciliation/executive ของ GAS ควรอ่านผ่าน `src/lib/gas/admin-analytics.ts` เท่านั้น; ถ้าจะเพิ่ม metric ใหม่ให้เพิ่มใน fact layer ก่อน ไม่คำนวณซ้ำใน route/page แต่ละตัว
24. **GAS Card Received Storage**: ตอนนี้ `cardReceived` ของ reconciliation ยังเก็บแฝงใน `shift.varianceNote` และรวมอยู่ใน `shift_reconciliations.transferReceived`; ทุก flow read/write ต้อง parse/build ผ่าน helper กลาง ห้ามแยก encode/decode เอง
25. **GAS Meter Reports**: `api/v2/gas/admin/reports/meters` ต้องอิง shift facts ชุดเดียวกับ daily/shift/reconciliation เพื่อให้ยอดมิเตอร์, transaction liters, และ liters variance ตรงกันทุกหน้า
26. **GAS Legacy Empty Open Shift**: ห้ามให้ `/gas-station/[id]/new/home` หรือ legacy `/api/gas-station/[id]/shifts` เปิดกะ GAS โดยไม่มี meter/gauge; ถ้าเจอ `OPEN` shift ที่ `meterRows=0` ให้ถือเป็น partial/corrupt row และ repair ผ่าน admin-confirmed flow ก่อนใช้งานต่อ
27. **GAS Daily Price Edits**: การแก้ราคาขายประจำวันของพนักงานต้องผ่าน `/api/v2/gas/[stationId]/price` เพื่อ update `dailyRecord.gasPrice/retailPrice/wholesalePrice` พร้อม audit; ห้ามแก้ผ่าน local state อย่างเดียว และต้องถือว่ารายการขายเดิมคง `pricePerLiter` เดิมไว้
28. **GAS Credit Bill Required Fields**: GAS `CREDIT` transaction ต้องมี `ownerId`, `truckId`, `billBookNo`, `billNo` และ backend ต้อง verify truck-owner relation ก่อนสร้าง transaction; ห้ามพึ่ง validation ฝั่งหน้าอย่างเดียว
29. **GAS Received Amount Validation**: ยอด `cashReceived`/`creditReceived`/`cardReceived`/`transferReceived` ตอนปิดกะต้องเป็นเลขไม่ติดลบทุกครั้ง; schema ยังไม่มี field `cardReceived` แยก จึงต้อง parse/build ผ่าน `buildGasVarianceNote`/fact layer กลางเท่านั้น
30. **GAS Open Shift Date Scope**: guard เปิดกะต้องเช็ก `OPEN` shift เฉพาะ station/day เดียวกับ `dateKey`; ห้ามให้กะค้างวันก่อนบล็อกการเปิดกะวันใหม่ ให้ใช้ `/api/admin/gas/stale-shifts` สำหรับ cleanup แบบมี audit
31. **GAS Orphan Transactions**: admin analytics ต้องไม่ทิ้ง transaction ที่ `shiftId=null` หรือ match shift window ไม่ได้; ให้แสดงเป็น `UNASSIGNED`/“ไม่ผูกกะ” เพื่อให้ผู้จัดการเห็นยอดจริง และ legacy sell route ต้อง block/auto-link ก่อนสร้างรายการใหม่
32. **GAS Amount-Based Sales**: หน้า GAS sell ต้องให้พนักงานกรอกยอดเงินเป็นหลัก และ backend ต้องคำนวณลิตรจาก `dailyRecord.gasPrice`; ห้ามเชื่อ `liters`/`pricePerLiter` จาก client เมื่อมี `amount` ส่งมา เพื่อไม่ให้ยอดขายกับราคาประจำวัน drift กัน
33. **GAS Single Source Entry Flow**: หน้า legacy `/gas-station/[id]` และทุกหน้า `/gas-station/[id]/new/*` ต้อง redirect ไป `/gas` v2; ห้ามเพิ่ม logic บันทึกขาย/มิเตอร์/สินค้า/สรุปใน legacy pages เพราะจะกลับไปสร้าง orphan/duplicate daily records และทำให้พนักงานสับสนระหว่างหน้าขาวกับหน้าดำ
...
48. **GAS Meter Report Received Amount**: ช่องยอดเงินจริงใน `/admin/gas/reports/meters` ต้องใช้ `shift.reconciliation.received` เมื่อมีกระทบยอดแล้ว เพราะ `shift.sales.total` เป็นยอดรวม transaction และอาจมีรายการซ้ำหรือรายการขายที่ยังต้องตรวจ เช่น วันที่ 2026-05-05 กะ 1 `station-5` มี transaction รวม 24,259 บาท แต่ยอดรับจริงตอนปิดกะคือ 15,842 บาท
47. **Tank Loy Mobile Receipt Original/Copy Cuts**: ใบเสร็จ/บิลเงินเชื่อ thermal ที่มีต้นฉบับ+สำเนาห้ามพึ่ง HTML `page-break-after` อย่างเดียวบน Android/Epson เพราะ roll print อาจพิมพ์ต่อกันยาวและไม่ใช้พื้นที่ 80mm เต็ม; ตั้งแต่ 2026-05-06 route receipt ใช้ `src/lib/thermal-receipt-print.ts` สร้าง ePOS XML direct ที่มี `<cut type="feed" />` หลังต้นฉบับและหลังสำเนา; หลังรูปหน้างานพบ direct print ชิดซ้ายตกขอบ ให้ใช้ text columns 42 สำหรับ 80mm / 30 สำหรับ 58mm พร้อม left padding 3/2 columns เพื่อจัดเข้ากลางกว่าเดิม
48. **Tank Loy Daily Summary Print Hierarchy**: mobile thermal daily summary ต้องเน้นยอดรวม, รวมลิตรขาย, ลิตรตามมิเตอร์, เลขมิเตอร์เปิด/ปิด, และผลต่างลิตรแบบ compact; ห้ามใช้ ePOS `width="2" height="2"` เพราะทำให้กระดาษยาวเกินหน้างาน; หลังรูปหน้างานพบ `em=true` หนาเกินอ่านยาก จึงห้ามใช้ ePOS emphasis ใน daily summary direct XML; ตัวเลขมิเตอร์เปิด/ปิดยาวจึงต้องคุมเองเป็นหัวจ่าย+ลิตรขาย 1 บรรทัด แล้วแยก `เปิด ...` / `ปิด ...` คนละบรรทัด ห้ามยัดเปิด-ปิดในบรรทัดเดียวจน printer wrap เอง; รายการย่อยอย่างจำนวนรายการ/สรุปชำระ/รายการเติม/เวลาพิมพ์ใช้ `font_b`; ส่วน admin daily summary classic page ต้องเรียก `printDailyWorkReport` แทน `window.print()` modal เพื่อให้ได้ A4 professional report template
...
56. **GAS Canonical Recovery Lock + Evidence**: canonical GAS recovery ต้อง re-fetch exact current OPEN Shift ก่อนเขียน, ใช้ backend `startBaselineLocked` เป็น source of truth และห้ามเปิดทางแก้ START หลังมี sales/end/reconciliation; meter/gauge rewrite ต้องส่ง photo URL เดิมกลับเมื่อไม่เลือกรูปใหม่เพื่อไม่ล้างหลักฐาน, END meter ต้องมี START และ `end >= start`; post-activity opening-meter repair ยังคงใช้ audited ADMIN route เดิม

## Changelog
- 2026-08-29: S98 ย้าย GAS meter/gauge recovery เข้า canonical Operations โดยคง exact-shift/baseline lock และ preserve existing photo evidence; isolated UAT ยืนยัน START หลัง END ถูก 409 ทั้ง meter/gauge
...
- 2026-05-06: follow-up mobile direct print จากรูปหน้างาน: ถอด ePOS `em=true` ออกจาก daily summary ทั้งใบให้ฟ้อนท์บางลง และปรับ receipt/credit direct print เป็น 42/30 columns พร้อม left padding 3/2 columns เพื่อไม่ชิดซ้ายตกขอบ
- 2026-05-06: follow-up mobile thermal daily summary: ปรับ hierarchy รอบหลังโดยทำยอดรวม/รวมลิตรขาย/ลิตรตามมิเตอร์/ผลต่างเป็น font หลักตัวหนา และลดจำนวนรายการ/สรุปชำระ/รายการเติม/เวลาพิมพ์เป็น `font_b` เพื่อให้จุดสำคัญเด่นขึ้นโดยไม่เพิ่มความยาวกระดาษ
