...
| `/station/1/v2` | canonical Overview / Operations / History | **KEEP_FULL_ADMIN_MAINTENANCE_COMPAT — S92 REVIEWED** | V2 is no longer a staff sales fallback. It remains ADMIN-only in canonical navigation while four maintenance slices move out: historical daily-price correction, historical meter/photo correction, transaction/slip/receipt maintenance, and audit/export/daily-print tooling. S92 removes the misleading FULL sales fallback and replaces the broken `/station/1/history` fallback with an ADMIN V2 maintenance entry. |
