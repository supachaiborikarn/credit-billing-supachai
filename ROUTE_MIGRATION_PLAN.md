...
| --- | --- | --- | --- |
| `/station/1/v2` | canonical Overview / Operations / History | **KEEP_FULL_ADMIN_MAINTENANCE_COMPAT — S95 PASS 1** | S93 moved daily-price correction and S94 moved meter/photo correction to canonical Operations. S95 pass 1 hardened historical transaction creation before UI migration: historical create is ADMIN-only, missing historical DailyRecords are not upserted, and FULL historical create requires an existing OPEN Shift without auto-creating one. V2 still owns transaction edit/void/slip/receipt maintenance plus audit/CSV/daily-print until the remaining S95 pass is complete. |
| `/station/1/v2` | canonical Overview / Operations / History | **KEEP_FULL_ADMIN_MAINTENANCE_COMPAT — S93 PARTIAL MIGRATION** | S93 moved daily retail/wholesale correction into canonical Operations, enforced historical price mutation as ADMIN-only, kept active STAFF current-day opening-price entry, and removed the V2 price-settings trigger (including the non-persisted `specialPrice` UI). V2 still owns three maintenance slices: historical meter/photo correction; transaction/slip/receipt maintenance; audit/CSV/daily-print tooling. |
